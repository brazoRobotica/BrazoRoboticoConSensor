...
int anglePinza = 20;
const int safeDistance = 4; // Define que tan cerca debe estar el objeto para que el brazo frene.
...
void setup() {
  ...
  pinMode(pinJoyButton1, INPUT_PULLUP); //Funcion busqueda
...
void scanForObject(Servo &servoBase, Servo &servoAltura, Servo &servoPinza, int safeDistance) {
    ...
    //servoBase.attach(7); // Volver a mandar señal al servo
...
/*
Idea (Ger)
for (int angle = 10; angle <= 170 and objetoDetectado; angle += 10)
